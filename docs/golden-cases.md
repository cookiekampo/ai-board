...
## Golden Case category fields

`docs/golden-cases.json` is still the source of truth. Existing `category` values are kept for compatibility, and new cases should also set these two fields:

- `workflowCategory`: `core`, `first-run`, `review`, `restore`, `prompt`, or `exit-card`
- `domainCategory`: `medical-kampo`, `ads-business`, `deep-research-meta`, `general`, or `uncategorized`

The browser Runner shows separate workflow and domain filters so cases remain easy to find as the list grows. The CLI also accepts category filters:

```bash
node scripts/check-golden-cases.mjs --list-categories
node scripts/check-golden-cases.mjs --all --workflow-category review
node scripts/check-golden-cases.mjs --all --domain-category medical-kampo
node scripts/check-golden-cases.mjs --category Review
node scripts/check-golden-cases.mjs --category Medical
```

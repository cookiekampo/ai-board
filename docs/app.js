...
## 自信度の理由`,
  ...
  review: [
    {
      role: "Review Framer / 対象分類・評価軸設計",
      title: "レビュー対象と評価軸の設計",
      target: "ChatGPT推奨",
      instruction: `対象をレビューする前に、今回のレビュー基準を設計してください。
必ず以下を含めてください。
- レビュー対象の種類
- 想定読者または利用者
- 成功条件
...
- 初期改善案
- 最初に直すべき箇所`,
      note: `ここでは改善案を作り込みすぎず、何を基準にレビューするかを明確にしてください。`
    },
    {
      role: "Critic / 対象別問題点レビュー",
      title: "対象別問題点レビュー",
      target: "Claude推奨",
      instruction: `Review Framerが定義したレビュー対象の種類・成功条件・評価軸に沿って、対象内容を批判的にレビューしてください。
必ず以下を含めてください。
- 致命的な問題
- 誤解されやすい点
- 抜け漏れ
...
- 修正すべき優先順位
- 追加で確認すべきこと`,
      note: `文章・仕様・プロンプト・アプリ機能・AI会議モードなど、対象に合った観点で批判してください。`
    },
    {
      role: "Revision Builder / 初回修正版作成",
      title: "初回修正版作成",
      target: "ChatGPT推奨",
      instruction: `Criticの指摘を踏まえて、初回修正版を作成してください。
必ず以下を含めてください。
- 残す要素
- 削る要素
- 追加する要素
...
- 文章レビューなら、初回修正版文章を出す
- 仕様レビューなら、修正版仕様案を出す
- プロンプトレビューなら、初回修正版プロンプトを出す
- アプリ機能レビューなら、Codexへの実装指示案を出す
- AI会議モードレビューなら、再設計案とStep別ロール案を出す`,
      note: `方針だけで終わらせず、可能な範囲で実際の成果物を作ってください。ただし過剰修正は避けてください。`
    },
    {
      role: "Critic / 修正版の副作用レビュー",
      title: "初回修正版の副作用レビュー",
      target: "Claude推奨",
      instruction: `Revision Builderが作成した初回修正版を再レビューしてください。
必ず以下を含めてください。
- 修正版で改善された点
- まだ残っている問題
- 過剰修正のリスク
...
- 追加で直すべき点
- 修正しない方がよい点
- Final Builderで反映すべき点`,
      note: `初回修正版そのものを見て、副作用・過剰修正・残存問題を重要度順に整理してください。`
    },
    {
      role: "Judge / 判定者",
      title: "最終化条件と未解決論点",
      target: "ChatGPTまたはClaude推奨",
      instruction: `ここまでのレビューを整理し、最終化できる状態かを判断してください。
必ず以下を含めてください。
- 現時点の有力案
- 却下候補
- 最終化してよい点
...
- 未解決論点
- 追加確認事項
- Final Builderに反映すべき指示
- 判断理由
- 暫定結論`,
      note: `単なる中間要約ではなく、最終修正版を作れる状態かどうかを判定してください。`
    },
    {
      role: "Final Builder / 最終修正版作成",
      title: "最終修正版作成",
      target: "ChatGPT推奨",
      instruction: `Step 4とStep 5の指摘を反映して、最終成果物を作成してください。
必ず以下を含めてください。
- 最終成果物
- 初回修正版から変更した点
- あえて変更しなかった点
...
- 仕様レビューなら、最終版仕様を出す
- プロンプトレビューなら、最終版プロンプトを出す
- アプリ機能レビューなら、Codex実装指示案を出す
- AI会議モードレビューなら、最終再設計案とロールプロンプト案を出す`,
      note: `議論のまとめではなく、Step 4とStep 5を反映した最終成果物を作ってください。`
    },
    {
      role: "Judge / 最終判定者",
      title: "最終結論・実装指示・テスト観点",
      target: "ChatGPTまたはClaude推奨",
...
## 結論の自信度
## 自信度の理由`,
      note: `最終結論は実行可能な形にしてください。レビュー品質チェックでは、致命的問題を見つけられたか、最終成果物が出たか、過剰修正を避けられたかも確認してください。`
    }
  ],
  deepResearchPrompt: [
    {

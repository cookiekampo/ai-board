...
ここまで
## 矛盾検出
Decision Ledgerと完成プロンプトが矛盾している場合は、矛盾内容を明示し、Decision Ledgerを優先して修正してください。矛盾がない場合は「なし」と書いてください。
## 避けるべき出力
## 次アクション
## 追加調査案
### 安全性を補う調査
...
Decision Ledgerで確定済みの対象読者、用途、外部公開の有無、初回調査範囲、除外範囲、深掘り項目、後続調査項目は再質問しないでください。
### 任意確認
必要な場合のみ。
### 未回答の場合の仮置き
...
回帰テスト観点:
Decision Ledgerに「対象読者: 漢方医向け」「用途: 内部資料」「外部公開の有無: 外部公開しない」「初回調査範囲: 医学的基礎 / 安全確認 / 症状クラスター / 漢方病態 / 代表処方群・生薬候補の概観」「後続調査に回す項目: 方剤別PMDA照合 / 症例報告 / エビデンスレビュー」「除外範囲: 患者配布 / Web公開 / 販売促進」がある場合、完成プロンプトは必ずこの条件に従ってください。`,
...
<!-- AI_BOARD:DR_REVIEW_ADDITIONAL_PROMPT:START -->
追加Deep Researchプロンプト案
<!-- AI_BOARD:DR_REVIEW_ADDITIONAL_PROMPT:END -->
...
function getDecisionLedgerLabelMap() {
  return deepResearchDecisionLedgerFields.reduce((map, field) => {
...
function inferDecisionLedgerFromSteering(note) {
  ...
  if (/漢方医/u.test(text)) addDecisionLedgerValue(ledger, "audience", "漢方医向け");
  else if (/医師/u.test(text)) addDecisionLedgerValue(ledger, "audience", "医師向け");
  if (/薬剤師/u.test(text)) addDecisionLedgerValue(ledger, "audience", "薬剤師向け");
  if (/漢方相談員|相談員/u.test(text)) addDecisionLedgerValue(ledger, "audience", "漢方相談員向け");
  if (/一般(?:ユーザー|患者|向け)/u.test(text)) addDecisionLedgerValue(ledger, "audience", "一般ユーザー向け");

  if (/内部資料|内部研修|社内資料|院内資料/u.test(text)) {
    addDecisionLedgerValue(ledger, "use", "内部資料");
  ...
  if (/初回|最初から|初回から|3\s*は\s*初回/u.test(text)) {
    if (/処方|生薬|証|クラスター|病態/u.test(text) || /3\s*は\s*初回/u.test(text)) {
      addDecisionLedgerValue(ledger, "initialScope", "医学的基礎 / 安全確認");
      addDecisionLedgerValue(ledger, "initialScope", "症状クラスター / 漢方病態 / 代表処方群・生薬候補の概観");
      addDecisionLedgerValue(ledger, "followUp", "方剤別PMDA照合 / 症例報告 / エビデンスレビュー");
    }
  ...
  if (/方剤別|PMDA|添付文書/u.test(text)) addDecisionLedgerValue(ledger, "followUp", "方剤別PMDA照合");
  if (/症例報告/u.test(text)) addDecisionLedgerValue(ledger, "followUp", "症例報告");
  if (/エビデンス|RCT|レビュー論文|システマティックレビュー/u.test(text)) addDecisionLedgerValue(ledger, "followUp", "エビデンスレビュー");
...
function buildDeepResearchPromptCompleteParts() {
  ...
  return {
    ...
    additional,
    questions,
    assumptions: extractAiBoardBlock(full, "DR_PROMPT_ASSUMPTIONS") || extractMarkdownSubsection(full, ["未回答の場合の仮置き"]),

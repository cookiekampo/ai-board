...
function buildDeepResearchReviewCompleteParts() {
  ...
  return {
    ...
    fixes: extractDeepResearchReviewSection(full, ["修正すべき内容"]),
...
function buildDeepResearchPromptCompleteParts() {
  ...
  const oneShot = extractMarkdownSubsection(full, ["一発版", "一発版プロンプト", "一発版Deep Researchプロンプト"]);
  const split = extractMarkdownSubsection(full, ["分割版", "分割版プロンプト", "分割版Deep Researchプロンプト"]);
  const additional = extractMarkdownSubsection(full, ["追加調査案", "追加Deep Researchプロンプト案", "追加Deep Researchプロンプト"]);
  const questions = extractMarkdownSubsection(full, ["ユーザーへの確認質問"]);
  return {
    full,
    completePrompt,
    oneShot: oneShot || completePrompt,
    split,
    order: extractMarkdownSubsection(full, ["推奨する調査構成", "推奨する実行順", "次アクション"]),
    additional,
    questions,
    assumptions: extractMarkdownSubsection(full, ["未回答の場合の仮置き"]),
    kampoBaseline: extractMarkdownSubsection(full, ["医学的基礎プロンプト", "病名の医学的基礎・医療連携", "医学的基礎"]),
    kampoPattern: extractMarkdownSubsection(full, ["漢方病態・証プロンプト", "漢方医学的病態把握", "漢方医学的病態", "証の整理"]),
    kampoFormula: extractMarkdownSubsection(full, ["処方・生薬・症状クラスター整理プロンプト", "処方・生薬・証・症状クラスター整理", "改善仮説を作る調査"]),
    kampoSafety: extractMarkdownSubsection(full, ["方剤別安全性・添付文書照合プロンプト", "方剤別安全性・添付文書照合", "安全性を補う調査"]),
    kampoPublic: extractMarkdownSubsection(full, ["一般ユーザー向け相談前メモ化プロンプト", "一般ユーザー向け相談前メモ化", "実用性を高める調査"]),
    kampoProfessional: extractMarkdownSubsection(full, ["専門職向け分冊化プロンプト", "専門職向け分冊化", "学習価値を高める調査"])
  };
}
